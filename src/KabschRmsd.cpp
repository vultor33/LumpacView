...
double KabschRmsd::rmsOverlay(string molName1, string molName2)
{
...
double KabschRmsd::rmsOverlay(vector<CoordXYZ> & mol1, vector<CoordXYZ> & mol2)
{
	double rmsd = 0;
	const int nlist = mol1.size();
	double(*mol1Array)[3] = new double[nlist][3];
	double(*mol2Array)[3] = new double[nlist][3];
	double(*saveMol1Array)[3] = new double[nlist][3];
	double(*saveMol2Array)[3] = new double[nlist][3];

	for (int i = 0; i < nlist; i++)
	{
		mol1Array[i][0] = mol1[i].x;
		mol1Array[i][1] = mol1[i].y;
		mol1Array[i][2] = mol1[i].z;
		mol2Array[i][0] = mol2[i].x;
		mol2Array[i][1] = mol2[i].y;
		mol2Array[i][2] = mol2[i].z;
		saveMol1Array[i][0] = mol1[i].x;
		saveMol1Array[i][1] = mol1[i].y;
		saveMol1Array[i][2] = mol1[i].z;
		saveMol2Array[i][0] = mol2[i].x;
		saveMol2Array[i][1] = mol2[i].y;
		saveMol2Array[i][2] = mol2[i].z;
	}
...
vector<CoordXYZ> KabschRmsd::readCoord(string fName)
{
...
void KabschRmsd::normalize(double a[3])
{
  double  b;
...
double KabschRmsd::dot(double a[3], double b[3])
{
...
void KabschRmsd::cross(double a[3], double b[3], double c[3])
{
  a[0] = b[1]*c[2] - b[2]*c[1];
  a[1] = b[2]*c[0] - b[0]*c[2];
  a[2] = b[0]*c[1] - b[1]*c[0];
 ...
 */
void KabschRmsd::setup_rotation(double ref_xlist[][3],
                    double mov_xlist[][3], 
                    int n_list,
                    double mov_com[3],
                    double mov_to_ref[3],
 ...
 */
int KabschRmsd::jacobi3(double a[3][3], double d[3], double v[3][3], int* n_rot)
{
  int count, k, i, j;
 ...
 */
int KabschRmsd::diagonalize_symmetric(double matrix[3][3],
 ...
 */
int KabschRmsd::calculate_rotation_matrix(double R[3][3],
                              double U[3][3], 
                              double E0,
                              double* residual)
{
...
void KabschRmsd::calculate_rotation_rmsd(double ref_xlist[][3],
                             double mov_xlist[][3], 
                             int n_list,
                             double mov_com[3],
                             double mov_to_ref[3],
...
void KabschRmsd::fast_rmsd(double ref_xlist[][3],
               double mov_xlist[][3], 
               int n_list,
               double* rmsd)
{ 
